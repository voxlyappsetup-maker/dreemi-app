...
- Phase 4-D3K status: story-generation monthly-limit plan decision is wired in `services/api/src/middleware/plans.middleware.ts` only.
- Phase 4-D3K status: User.plan remains compatibility projection input via `getPlanForAccessCheck(userId, user.plan)`, while monthly counting logic remains in middleware.
- Phase 4-D3K status: `children.ts` remains D3G wiring and is unchanged; `stories.ts` remains non-wired directly; payments/checkout/webhook remain non-wired.
